...
The current server reports `protocol:12` from `identify` and `ping`. Clients must inspect `identify.protocol` before using versioned additions. A client selecting `attach-surface` with `mode:"render"` must require protocol 7; stable split ids and `set-split-ratio` require protocol 8; stack layouts, `new-pane`, and `clear-history` require protocol 9; per-surface client sizing requires protocol 10; terminal creation and `run` lifecycle response shapes, terminal renderer minting, and upstream viewport/layout additions require protocol 11; and lifecycle readiness, agent telemetry/events, the `error` agent state, plus notification subtitles require protocol 12. Additive commands also require their advertised capabilities: `daemon-handoff-force-v1` for `shutdown-daemon` with `force:true`, `browser-pointer-frame-guard-v1` for browser pointer routing and browser `attach-surface` admission, `viewport-splits-v1` for `new-pane-right`, `viewport-column-resize-v1` for `set-viewport-pane-width`, `layout-undo-v1` for `undo-layout`, `clear-history-v1` for `clear-history`, and `clear-history-key-v1` for its structured fallback. A browser attach connection must advertise `browser-pointer-frame-guard-v1` through `set-client-info` on that same socket before attaching.
